import { FormData, buildConnector, request } from 'undici';
    ...
    async checkLogin(retries = 3){
        ...
        try{
            ...
            if(regionPrefix){
                ...
                const newOrigin = new URL(newHostname).origin;

                // FIX: only recurse if the hostname is actually changing AND
                // we still have retries left. The original code recursed
                // unconditionally, which loops forever if TeraBox keeps
                // sending the same (or bouncing) region-domain-prefix header.
                if(this.params.whost !== newOrigin && retries > 0){
                    console.warn(`[WARN] Default hostname changed to ${newOrigin}`);
                    this.params.whost = newOrigin;
                    return await this.checkLogin(retries - 1);
                }

                if(retries <= 0){
                    console.error('[ERROR] checkLogin: too many region-domain-prefix redirects, giving up.');
                    throw new Error('checkLogin', { cause: 'Too many region redirects (possible invalid/flagged cookie)' });
                }
                // Hostname didn't actually change -- fall through and parse
                // the response body normally instead of looping.
            }
